...
//https://github.com/THaGKI9/merkletree/blob/main/merkletree.go
import (
	"bytes"
	"errors"
	"fmt"
	...
	for len(nodesInLayer) > 1 {
		...
		for i := 0; i < layerSize; i += 2 {
			...
			if i+1 == layerSize {
				...
				newNode.Hash, err = HashOfTwoBytes(opts.HashObj, newNode.Left.Hash, newNode.Right.Hash)
...
func Verify(root []byte, leaf []byte, proof [][]byte, hashObj hash.Hash) bool {
	...
	for _, hash := range proof {
		left := calculatedRoot
		right := hash
		calculatedRoot, err = HashOfTwoBytes(hashObj, left, right)
		if err != nil {
			return false
		}
	}
